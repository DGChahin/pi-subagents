...
A [pi](https://pi.dev) extension that brings **Claude Code-style autonomous sub-agents** to pi. Spawn specialized agents that run in isolated sessions — each with its own tools, system prompt, model, and thinking level. Agent tool calls and resumes run in background, while explicit result retrieval, steering, and custom agent types remain available.
...
- **Session resume** — pick up where an agent left off in background, return its existing ID immediately, preserve full conversation context, share the normal background concurrency queue, and notify on completion like a fresh spawn
...
- **Git worktree isolation** — run agents in isolated repo copies; changes are checkpointed to branches while the worktree remains available for resume
...
Agent calls run in background, return an ID immediately, and send a concise completion callback with a result preview. An explicit `run_in_background: false` is rejected. A successful spawn returns `terminate: true`, so an all-Agent tool batch can end the parent run and reach idle. Pi waits until every finalized result in the parallel batch is terminating. Any rejected call stays non-terminating so the parent can correct it. Use `get_subagent_result` when you need full stored output.
...
**Background-only compatibility** (`backgroundByDefault`): retained only so upstream/global settings files remain readable. This fork canonicalizes it to `true` in settings lifecycle events and on the next settings write. Setting it to `false` does not restore foreground top-level execution: `Agent` calls, resumes, mentions, manager-registry calls, and RPC spawns remain detached, and explicit `run_in_background: false` is rejected. Nested subagents keep their separate foreground-by-default behavior.
...
The agent gets a full, isolated copy of the repository. Each completed turn checkpoints the worktree without removing it, so a resumed agent continues in the same copy:
- **No changes:** no branch is created; the unchanged worktree remains available for resume
- **Changes made:** changes are committed to a new branch (`pi-agent-<id>`), and the result names the branch and the `git merge` command for it
...
The retained worktree is removed when its resumable record is evicted, cleared on a session change, or disposed at shutdown. Its result branch remains. If checkpoint or cleanup fails, the result identifies the retained path for manual recovery.
