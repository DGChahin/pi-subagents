...
export async function runMentionClone(opts: MentionCloneOptions): Promise<MentionCloneResult> {
  ...
  const cloneAgentTool: ToolDefinition = {
    ...
    execute: (_cloneToolCallId, params, signal, onUpdate, _cloneCtx) => {
      ...
      spawned = true;
      // undefined tool-call id + the main ctx: see the header. Pass the required
      // background spelling explicitly rather than relying on compatibility
      // defaults: this fork never permits a foreground top-level clone spawn,
      // whose result would disappear with the throwaway session.
