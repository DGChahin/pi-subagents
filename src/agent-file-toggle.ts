...
export function serializeAgentFile(cfg: AgentConfig): string {
  ...
  // Preserve both values for format symmetry and nested-agent compatibility.
  // At this fork's top level omission/true run detached and false is rejected;
  // serializing false must not be read as re-enabling a foreground top-level run.
