...
interface ResolveOptions {
   ...
   *
   * This fork's top-level callers pass `true` unconditionally; the retained
   * `backgroundByDefault` setting is compatibility-only and cannot restore a
   * blocking top-level run. Nested callers pass `false`: a detached child is
   * killed when its parent settles and has no notification path of its own.
   * The fallback only covers callers that supply no options (in-tree: tests).
   */
