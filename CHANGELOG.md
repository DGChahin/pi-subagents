...
### Changed
- **Tool execution is represented by activity cards instead of separate TUI transcript rows** ([#251](https://github.com/tintinweb/pi-subagents/pull/251)). Built-in tools, pi-subagents tools, and tools owned by unrelated extensions now keep their execution and model-context behavior while their individual call/result rows stay hidden; the active card continues to show tool names, counts, step progress, streamed status, usage, duration, and the final outcome. Pi does not yet expose a supported renderer-only override for foreign tools, so this temporarily patches the exported `ToolExecutionComponent.render()` method only during active TUI sessions and restores it on session switch or shutdown. Another extension can bypass suppression if it later replaces that same method without delegating. The relevant [Pi issue #8347](https://github.com/earendil-works/pi/issues/8347) and unmerged [Pi PR #8343](https://github.com/earendil-works/pi/pull/8343) were auto-closed by Pi's contribution-policy bot and have not been accepted upstream; this compatibility workaround will be removed after Pi releases and pi-subagents adopts an equivalent official API.
- **BREAKING: Top-level agents now dispatch and resume only in the background, with idle-aware completion callbacks** ([#1](https://github.com/DGChahin/pi-subagents/pull/1)). The `Agent` tool no longer waits inline for a top-level run: fresh dispatches default to background execution, an explicit `run_in_background: false` is rejected (including an agent configuration that defaults it to false), and `resume` queues another background turn on the existing record and returns the same agent ID immediately. Callers now receive concise completion callbacks only after the parent is idle; completions that become ready together are coalesced into one follow-up turn, while full output remains stored behind `get_subagent_result` and a successful explicit retrieval suppresses its redundant callback. Before a callback restarts the parent, the extension offers context compaction a barrier: claimed callbacks remain held until the matching compactor session and generation reports success or failure, and an invalidated barrier discards callbacks for that stale parent generation rather than delivering them into another session. A top-level spawn records its parent-session generation at dispatch, before any concurrency queue delay, and a resumed record replaces that identity when the resume is dispatched, so queued work remains attached to the session that requested it instead of whichever session is active when a slot opens. Background resumes now share the normal `maxConcurrent` pool with fresh background agents, can remain queued until capacity is available, reject another resume while that record is already queued or running, and release their slot and drain the queue through the same completion path as a fresh run.
