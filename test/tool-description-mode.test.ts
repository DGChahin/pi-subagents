...
describe("toolDescriptionMode", () => {
  ...
  it("pins the exact background-only guidance in the built-in description and shipped template", () => {
    const full: string = setup({ toolDescriptionMode: "full" }).get("Agent").description;
    ...
    const template = readFileSync(EXAMPLE_TEMPLATE, "utf-8");
  ...
  describe("worktreeIsolation gates the isolation parameter and its prose", () => {
    ...
    it("compact mode says nothing about isolation when disabled", () => {
      ...
      expect(tools.get("Agent").description).toContain(
        "resume continues a previous agent in background and returns the same ID",
      );
    });
  });
