  ...
  it("does not inject nested orchestration tools into a custom agent by default", async () => {
    ...
    ({ run, cwd } = await runWithAgents(
      ...
      (ctx) => {
        ...
        const fetched = toolResults(ctx, "get_subagent_result");
    ...
    try {
      ({ run, cwd } = await runWithAgents(
        ...
        async (ctx) => {
          ...
          // probe must be launched in one parallel tool batch while the root is
          // active. Async join lets the probe callback arrive independently.
          return [
            agentCall({
              subagent_type: "owner",
