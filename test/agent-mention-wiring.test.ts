...
/** A runAgent that stays running until extension shutdown aborts it. */
...
describe("messaging a running agent", () => {
  ...
  it("addresses same-type siblings by their numbered handles", async () => {
    ...
    vi.mocked(runAgent)
      .mockImplementationOnce((_c: any, _t: any, _p: any, o: any) => holdUntilAbort(first, o))
      .mockImplementationOnce((_c: any, _t: any, _p: any, o: any) => holdUntilAbort(second, o));
...
describe("messaging a finished agent", () => {
  ...
  it("does not attribute the new answer to the tool call that spawned it", async () => {
    // A mention-resume has no tool call behind it, so leaving the spawning
  ...
  it("relays the resumed answer through the ordinary completion notification", async () => {
    ...
    expect(pi.sendMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        content: expect.stringContaining("Stored. Use get_subagent_result"),
        details: expect.objectContaining({ resultPreview: "second answer" }),
      }),
...
describe("mentioning an agent that has never run", () => {
  ...
  it("wires run activity callbacks for a mention spawn", async () => {
    ...
    // callbacks that feed FleetView and the activity-card store.
    const { lifecycle } = bootDirect();
    heldRun(fakeSession());
...
describe("@agent-<type> — Claude Code's manual spelling", () => {
  ...
  it("prefers an agent literally named agent-<x> over the unwrapped spelling", async () => {
    ...
    vi.mocked(runAgent)
      .mockImplementationOnce((_c: any, _t: any, _p: any, o: any) => holdUntilAbort(plain, o))
