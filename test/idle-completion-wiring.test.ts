...
describe("top-level background lifecycle and idle completion delivery", () => {
  ...
  it("coalesces every idle pending ID behind exact barriers and discards an invalidated generation", async () => {
    vi.useFakeTimers();
    const harness = boot({ defaultJoinMode: "async" });
    let idle = false;
    const context = makeCtx("session-1", () => idle);
    await harness.lifecycle.get("session_start")?.({}, context);
    vi.mocked(runAgent).mockImplementation((_ctx, _type, prompt) =>
      Promise.resolve(agentResult(`result:${prompt}`)) as never,
    );

    const claims = [
      { barrierId: 10, sessionId: "compactor", generation: 7 },
      { barrierId: 11, sessionId: "compactor", generation: 8 },
    ];
    harness.pi.events.on("context-compact:before-continuation", (value: unknown) => {
      const claim = claims.shift();
      if (!claim) return;
      Object.assign(value as object, { hold: true, claimedBy: "context-compact", ...claim });
    });

    const invalidatedIds = [
      idFrom(await spawn(harness.tools, "invalidated-a", context)),
      idFrom(await spawn(harness.tools, "invalidated-b", context)),
    ];
    await microflush();
    ...
    idle = true;
    harness.lifecycle.get("agent_settled")?.({}, context);
    for (const mismatch of [
      { barrierId: 999, sessionId: "compactor", generation: 7 },
      { barrierId: 10, sessionId: "other-compactor", generation: 7 },
      { barrierId: 10, sessionId: "compactor", generation: 999 },
    ]) {
      harness.pi.events.emit("context-compact:barrier-open", {
        ...mismatch,
        outcome: "compacted",
      });
      expect(harness.pi.sendMessage).not.toHaveBeenCalled();
    ...
    const manager = (globalThis as Record<symbol, unknown>)[Symbol.for("pi-subagents:manager")] as {
      getRecord(id: string): { pendingDeliveryRevision?: number } | undefined;
    };
    for (const id of invalidatedIds) {
      expect(manager.getRecord(id)?.pendingDeliveryRevision).toBeUndefined();
    }

    const pendingIds = [
      idFrom(await spawn(harness.tools, "pending-a", context)),
      idFrom(await spawn(harness.tools, "pending-b", context)),
    ];
    await microflush();
    await vi.advanceTimersByTimeAsync(200);
    expect(harness.pi.sendMessage).not.toHaveBeenCalled();
    ...
    for (const id of pendingIds) expect(content).toContain(`<task-id>${id}</task-id>`);
