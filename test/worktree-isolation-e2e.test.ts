 ...
 * the main checkout → settlement checkpoints them to a branch the result names
 * → host teardown removes the retained resumable copy. Every link was tested;
 * the chain was not.
 *
...
/** Full child output the parent fetched after the completion callback. */
function subagentResultText(session: Context): string {
...
function respondSpawning(isolation: "worktree" | undefined): (context: Context) => FauxReply {
  return (context: Context): FauxReply => {
    ...
    // Parent: dispatch once, then use the real callback's task ID to retrieve
    // the completed worktree result.
    if (toolResultNames(context).includes("get_subagent_result")) {
      return `parent saw: ${subagentResultText(context)}`;
    }
...
describe("worktree isolation e2e (real git, real pi-mono, faux model)", () => {
  ...
  it("runs the child in the copy and lands its changes on a branch, not the main checkout", async () => {
    ...
    const result = subagentResultText(run.parentSession);
    ...
    // Background worktrees remain resumable after settlement. Host teardown is
    // the definitive cleanup boundary and removes the retained copy.
