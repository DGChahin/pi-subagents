 ...
 * tokens would claim the run was measured and free. Each supported surface
 * punctuates its own — notification stats join with `·`, while the
 * `get_subagent_result` header joins with `|` — so cost travels as a number and
 * is formatted at the end rather than being baked into the token string.
 */
...
/** One background run that spends `cost` on a single assistant message. */
...
describe("cost display", () => {
  ...
  describe("the background completion notification the model reads", () => {
    ...
    async function activate(lifecycle: Map<string, any>) {
      await lifecycle.get("session_start")?.({}, ctx({ isIdle: () => true }));
    }
