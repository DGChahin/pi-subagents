...
describe("Agent tool — background resume wiring", () => {
  ...
  it("rejects an explicit top-level foreground resume without starting it", async () => {
    const { pi, tools, lifecycle } = makePi();
    subagentsExtension(pi);
    const ctx = makeCtx(cwd);
    const id = await spawnSettled(tools, ctx);
    ...
    expect(resultText(res)).toContain("Foreground Agent resume is disabled");
    expect(resultText(res)).toContain("get_subagent_result");
